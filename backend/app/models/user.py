...
class User(Base):
    ...
    username: Mapped[str] = mapped_column("Username", NVARCHAR(50), unique=True, nullable=False)
    password_hash: Mapped[str] = mapped_column("PasswordHash", NVARCHAR(255), nullable=False)
    full_name: Mapped[str] = mapped_column("FullName", NVARCHAR(100), nullable=False)
    phone: Mapped[str | None] = mapped_column("Phone", NVARCHAR(15), nullable=True)
    email: Mapped[str | None] = mapped_column("Email", NVARCHAR(100), nullable=True)
    role: Mapped[str] = mapped_column("Role", NVARCHAR(20), nullable=False)
